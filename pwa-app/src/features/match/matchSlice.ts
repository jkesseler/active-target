...
export const timerReset = createAction<{}>('match/timerReset');
...
export const matchSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(timerReset, (state, { payload }) => {
        const match = state.matches.find(m => m.id === state.currentMatchId);
        if (match?.currentRun) {
          match.currentRun.timer = {
            elapsedTime: 0,
...
export const selectCurrentMatch = (state: AppRootState) =>
  state.match.matches.find(m => m.id === state.match.currentMatchId);
export const selectCurrentStageId = (state: AppRootState) =>
  selectCurrentMatch(state)?.currentStageId;
export const selectCurrentShooterId = (state: AppRootState) =>
  selectCurrentMatch(state)?.currentShooterId;
export const selectCurrentRun = (state: AppRootState) =>
  selectCurrentMatch(state)?.currentRun;
export const selectCurrentTimer = (state: AppRootState) =>
  selectCurrentRun(state)?.timer;export const { setCurrentMatch, addMatch, updateMatch } = matchSlice.actions;
